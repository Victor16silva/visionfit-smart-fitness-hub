...
export default function EditWorkout() {
  ...
  const loadWorkout = async () => {
    ...
    setLoadingData(true);
    try {
      const { data: workout, error } = await supabase.from("workout_plans").select("*").eq("id", id).single();
      ...
      // Load exercises
      const { data: workoutExercises } = await supabase.from("workout_exercises").select(`id, exercise_id, sets, reps_min, rest_seconds, order_index, exercises (id, name, muscle_groups)`).eq("workout_plan_id", id).order("order_index");
      if (workoutExercises) {
        setExercises(workoutExercises.map((we: any) => ({ id: we.exercise_id, workoutExerciseId: we.id, name: we.exercises?.name || "Exercício", sets: we.sets, reps: we.reps_min, restSeconds: we.rest_seconds, isExpanded: false, isNew: false })));
      }
    } catch (error) { console.error(error); toast.error("Erro ao carregar treino"); } finally { setLoadingData(false); }
  };
  ...
  const updateExerciseConfig = async (exId: string, field: string, value: number) => {
    setExercises(prev => prev.map(ex => ex.id === exId ? { ...ex, [field]: value } : ex));
    const exercise = exercises.find(e => e.id === exId);
    if (exercise?.workoutExerciseId) {
      const updateData: any = {};
      if (field === 'sets') updateData.sets = value;
      if (field === 'reps') { updateData.reps_min = value; updateData.reps_max = value; }
      if (field === 'restSeconds') updateData.rest_seconds = value;
      await supabase.from("workout_exercises").update(updateData).eq("id", exercise.workoutExerciseId);
    }
  };
  const moveExercise = async (index: number, direction: 'up' | 'down') => {
    ...
    for (let i = 0; i < newExercises.length; i++) {
      if (newExercises[i].workoutExerciseId) {
        await supabase.from("workout_exercises").update({ order_index: i }).eq("id", newExercises[i].workoutExerciseId);
      }
    }
  };
  const removeExercise = async (exId: string) => {
    const exercise = exercises.find(e => e.id === exId);
    if (exercise?.workoutExerciseId) {
      await supabase.from("workout_exercises").delete().eq("id", exercise.workoutExerciseId);
  ...
  const addExercise = (ex: Exercise) => {
    if (!exercises.find(e => e.id === ex.id)) {
      setExercises(prev => [...prev, { id: ex.id, name: ex.name, sets: 4, reps: 8, restSeconds: 60, isExpanded: false, isNew: true }]);
    }
    setShowPicker(false);
  };

  const handleSave = async () => {
    if (!formData.name || !id) { toast.error("Preencha o nome do treino"); return; }
    setLoading(true);
    try {
      const muscleGroups = formData.focusArea ? [formData.focusArea] : ["Geral"];
      await supabase.from("workout_plans").update({ name: formData.name, description: formData.focus, muscle_groups: muscleGroups, category: formData.category, division_letter: formData.division, duration_minutes: parseInt(formData.duration) || 40, calories: parseInt(formData.calories) || 200, total_weight_kg: parseFloat(formData.totalWeight) || 0, cover_image_url: formData.coverImageUrl || null, is_recommended: formData.isRecommended, is_daily: formData.isDaily, is_challenge: formData.isChallenge, challenge_points: formData.isChallenge ? parseInt(formData.challengePoints) || 50 : 0 }).eq("id", id);
      // Insert new exercises
      const newExercises = exercises.filter(e => e.isNew);
      if (newExercises.length > 0) {
        const maxOrder = exercises.filter(e => !e.isNew).length;
        const inserts = newExercises.map((ex, i) => ({ workout_plan_id: id, exercise_id: ex.id, order_index: maxOrder + i, sets: ex.sets, reps_min: ex.reps, reps_max: ex.reps, rest_seconds: ex.restSeconds }));
        await supabase.from("workout_exercises").insert(inserts);
      }
      toast.success("Treino atualizado com sucesso!"); navigate("/dashboard");
    } catch (error) { console.error(error); toast.error("Erro ao atualizar treino"); } finally { setLoading(false); }
  };
    ...
    <div className="min-h-screen bg-background">
      <header className="border-b border-border bg-card/50 backdrop-blur sticky top-0 z-20">
        <div className="container mx-auto px-4 py-4 flex items-center gap-4">
          ...
          <h1 className="text-lg font-bold text-foreground">Editar Treino</h1>
        </div>
      </header>

      <main className="container mx-auto px-4 py-6 max-w-lg space-y-4">
        <div className="grid grid-cols-3 gap-3">
          <div className="col-span-2"><label className="block text-sm text-foreground mb-1.5">Nome do Treino *</label><Input value={formData.name} onChange={(e) => setFormData(prev => ({ ...prev, name: e.target.value }))} className="bg-muted border-border h-11" /></div>
          <div><label className="block text-sm text-foreground mb-1.5">Divisão *</label><Input value={formData.division} onChange={(e) => setFormData(prev => ({ ...prev, division: e.target.value }))} className="bg-muted border-border h-11" /></div>
        </div>
        <div><label className="block text-sm text-foreground mb-1.5">Foco</label><Input value={formData.focus} onChange={(e) => setFormData(prev => ({ ...prev, focus: e.target.value }))} className="bg-muted border-border h-11" /></div>
        ...
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-2"><Checkbox id="rec-edit" checked={formData.isRecommended} onCheckedChange={(c) => setFormData(prev => ({ ...prev, isRecommended: !!c }))} /><label htmlFor="rec-edit" className="text-sm text-foreground cursor-pointer">Recomendado</label></div>
          <div className="flex items-center gap-2"><Checkbox id="daily-edit" checked={formData.isDaily} onCheckedChange={(c) => setFormData(prev => ({ ...prev, isDaily: !!c }))} /><label htmlFor="daily-edit" className="text-sm text-foreground cursor-pointer">Treino Diário</label></div>
        </div>
        <div className="space-y-3">
          <div className="flex items-center gap-2"><Checkbox id="chal-edit" checked={formData.isChallenge} onCheckedChange={(c) => setFormData(prev => ({ ...prev, isChallenge: !!c }))} /><label htmlFor="chal-edit" className="text-sm text-amber-400 cursor-pointer">🏆 Marcar como Desafio</label></div>
        ...
        <div>
          ...
          {exercises.length > 0 && <div className="space-y-2 mb-3">{exercises.map((ex, index) => (
            <div key={`${ex.id}-${index}`} className={`rounded-lg border overflow-hidden ${ex.isNew ? 'bg-lime/10 border-lime/30' : 'bg-muted/50 border-border'}`}>
              <div className="flex items-center justify-between p-3 cursor-pointer" onClick={() => toggleExerciseExpanded(ex.id)}>
                <div className="flex items-center gap-3">
                  ...
                  <div><p className="text-sm font-medium text-foreground">{ex.name}</p><p className={`text-xs ${ex.isNew ? 'text-lime' : 'text-muted-foreground'}`}>{ex.isNew ? 'Novo • ' : ''}{ex.sets} séries • {ex.reps} reps • {ex.restSeconds}s</p></div>
                </div>
        ...
        <Button className="w-full bg-lime text-black font-bold h-12 hover:bg-lime/90" onClick={handleSave} disabled={loading}><Dumbbell className="h-4 w-4 mr-2" />{loading ? "Salvando..." : "Salvar Alterações"}</Button>
      </main>

      {showPicker && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-card rounded-2xl w-full max-w-md max-h-[80vh] overflow-hidden">
            <div className="p-4 border-b border-border flex items-center justify-between"><h3 className="font-bold text-foreground">Selecionar Exercício</h3><button onClick={() => setShowPicker(false)} className="text-muted-foreground hover:text-foreground">✕</button></div>
            ...
            <div className="overflow-y-auto max-h-[50vh] p-4 pt-0 space-y-2">
              {filteredExercises.map((ex) => (
                <button key={ex.id} onClick={() => addExercise(ex)} className="w-full flex items-center gap-3 p-3 rounded-lg bg-muted/50 hover:bg-muted text-left">
                  <div className="w-10 h-10 rounded-lg bg-amber-500/20 flex items-center justify-center"><Dumbbell className="h-5 w-5 text-amber-500" /></div>
